...
var rv = {
  ...
  go_to_playground: function(e, v) {
    go_to_playground(v.c['iso3']);
  }
}
...
function go_to_playground(iso3) {
  window.location.href = "./playground.html?iso3=" + iso3 + "&tier=3&diesel_price=nps";
}
...
function drawMap(country_arrangement) {
  ...
  .on({
    ...
    dblclick: function(d) {
      if (d.countryCode != 0 && d.countryCode != 'X') {
        go_to_playground(d['countryCode']);
      }
    }
...
queue()
  .defer(d3.csv, './data/countryarrangement.csv')
  .defer(d3.json, './data/countries_summaries.json')
